...
__interrupt void UART_ISR(void)
{
    ...
    else if(UCA1RXBUF == 0x5E)	// if message is "^", GPS is connected
    {
    	if(powerState(-1) == 1)	// if powerState is currently in search mode
    	{
    		powerState(2);
    		// Turn off LED indicator
    		P1OUT = P1OUT & ~(BLUE_LED);
...
void startGPS(void)
{
	...
	P2OUT = P2OUT | BIT2; 	// leave on for Bluetooth
}
