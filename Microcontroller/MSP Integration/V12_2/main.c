#include <msp430.h>
// Description: this is the twelth version, modifying gps code
// Authors: Chase
// Date last modified: 5/3/2017
...
#define POWER_ON BIT7				// pin used (2.7) as a button for powering on GPS and Bluetooth
#define BT_SEARCH_LIMIT 300000
#define RESET BIT6					// reset pin for GPS
...
void stopGPS(void);							// this will stop GPS module with end of pulse and turn off BT
...
signed int powerState(signed int nNewState);	// records power state of device
...
int main(void)
 {
    ...
    while(1){
    	if(powerState(-1) == 0)
    	{
    		// turn off LED
    		nColor = 0;
			P1OUT = P1OUT & ~(GREEN_LED | RED_LED | BLUE_LED);
			// turn off alert and motor
    		nAlertState = 0;
    		...
    		if( P2IN & POWER_ON ) // if button is pressed
    		{
    			// turn on GPS and Bluetooth
    			for(nCounter = 0; nCounter < 1000; nCounter++)
				{
					__delay_cycles(1000);			// wait 1ms
				}
				startGPS();
				// change power state to BT search mode
    			powerState(1);	// power is on, new power state
    			P1OUT = P1OUT | BLUE_LED;	// Turn on Blue LED to indicate BT search mode
    	...
    	// while in BT search mode
    	else if(powerState(-1) == 1)
    	{
    		if(nBTSearchTime > BT_SEARCH_LIMIT)
    		{
    			// Turn off LED indicator
    			P1OUT = P1OUT & ~(BLUE_LED);
    			// put power state back to low power mode
    			powerState(0);
    			// turn off BT and GPS
    			stopGPS();
    		}
    		// increment search time counter
    		nBTSearchTime++;
    	}
    	// if power state is in active mode
    	else if(powerState(-1) == 2)
    	{
...
__interrupt void UART_ISR(void)
{
	if(powerState(-1) == 2)
	{
		...
		else if(UCA1RXBUF == 0x7E)	// Bluetooth is closing connection ("~")
		{
			// turn off LEDs
			P1OUT = P1OUT & ~(GREEN_LED | RED_LED | BLUE_LED);
			// put power state back to low power mode
	...
	else if(powerState(-1) == 1)	// if in state 1
	{
		if(UCA1RXBUF == 0x5E)	// if message is "^", GPS is connected
		{
			powerState(2);
			// Turn off LED indicator
			P1OUT = P1OUT & ~(BLUE_LED);
...
void set_inputs_and_outputs(void)
{
    ...
    P2OUT = P2OUT & ~(BIT2 | POWER_ON);					// P2.2 low, P2.7 pull-down resistor
    P2REN = P2REN | POWER_ON;							// Enable pull-down resistor
    P2DIR = ( P2DIR | BIT2 ) & ~(POWER_ON);				// Declare P2.2 as output and 2.7 as input
...
// function used to put GPS and Bluetooth modules in sleep mode
void stopGPS(void)
{
	char sShutDown[8] = "$PSRF117";
	int nCounter = 0;
	int nInterval = 0;
	for(nCounter = 0; nCounter < 1; nCounter++)
	{
	...
	for(nCounter = 0; nCounter < 1; nCounter++)
	{
		__delay_cycles(50);			// wait 1ms
	}
	P2OUT = P2OUT & ~(BIT2);// end pulse
}

// function for keeping track of power state
signed int powerState(signed int nNewState)
{
	/*
	 * Value	| 	Power State
	 * 0		|	Low power mode
	 * 1		|	BT search mode
	 * 2		| 	BT connected, active mode
	 */
	static signed int nCurrentState = 0;
	if(nNewState != -1)
	{
