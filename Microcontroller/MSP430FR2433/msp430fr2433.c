...
void rgb(char RED, char GREEN, char BLUE)  // use to set up RGB function
{
	P1OUT = RED << 1 | GREEN << 2 | BLUE << 3;  // shifted to the correct position and the register
	                                            //P1OUT = 0b00000111; // turn them	                                            // and OR them altogether
}
...
int main(void)
{
	...
	// Declare pins
	P1DIR = 0b00000111;     // P1.2, P1.1, P1.0 as output
	                        // P1OUT = 0b00000111     turn them on
...
__interrupt void UART_ISR(void)
{
	...
	else if(UCA1RXBUF == 0x4F) // if message is "O" for yellow
	{
		nColor = 2;
		P1DIR = 0b00000111;
	}
